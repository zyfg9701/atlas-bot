...
#   powershell -ExecutionPolicy Bypass -File .\scripts\start-cli-stack.ps1
#   $env:ATLAS_AGENT_CLI='C:\path\to\agent.exe'; .\scripts\start-cli-stack.ps1
#   .\scripts\start-cli-stack.ps1 -Stream
#
# Env overrides: same names as scripts/dev-cli-stack.ps1
# Layout: <root>\bin\atlas-bot-gateway.exe + atlas-bot-hub.exe ; this file under <root>\scripts\
...
$GW_BIN = Join-Path $BinDir 'atlas-bot-gateway.exe'
$HUB_BIN = Join-Path $BinDir 'atlas-bot-hub.exe'
if (-not (Test-Path -LiteralPath $GW_BIN)) {
  $GW_BIN = Join-Path $BinDir 'atlas-bot-gateway'
}
if (-not (Test-Path -LiteralPath $HUB_BIN)) {
  $HUB_BIN = Join-Path $BinDir 'atlas-bot-hub'
}

if (-not (Test-Path -LiteralPath $GW_BIN)) {
  Write-Host "error: missing executable: $GW_BIN" -ForegroundColor Red
  Write-Host 'Run scripts\pack-dist.ps1 (or install-local) first. See dist\README-INSTALL.md' -ForegroundColor Red
  exit 1
}
if (-not (Test-Path -LiteralPath $HUB_BIN)) {
  Write-Host "error: missing executable: $HUB_BIN" -ForegroundColor Red
  exit 1
}
...
Mock bypass (dev tree only; not shipped in default dist):
  `$env:ATLAS_AGENT_CLI='<repo>\tools\mock-cli\mock-atlas-agent-cli.cmd'; .\scripts\start-cli-stack.ps1
...
Write-Host '==> CLI primary stack (dist binaries)'
Write-Host "    ROOT=$Root"
Write-Host "    gateway: $GW_BIN"
Write-Host "    hub:     $HUB_BIN"
